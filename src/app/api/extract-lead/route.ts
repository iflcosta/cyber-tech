...
${messages.map((m: any) => `${m.role}: ${m.content}`).join('\n')}`;
        ...
        // Limpar possíveis formatações markdown do retorno
        ...
        try {
            ...
            console.error("Falha ao parsear retorno do Gemini:", extractedDataText);
        }

        if (!extractedData.name && !extractedData.phone) {
            return NextResponse.json({ status: 'no_lead_data_extracted' }, { status: 200 });
        }
        ...
        if (extractedData.phone) {
            ...
            // 3. Inserir no Supabase se for novo
            // O voucher será gerado por um trigger no Supabase ou podemos gerar aqui
        ...
        return NextResponse.json({ status: 'no_phone_provided' }, { status: 200 });

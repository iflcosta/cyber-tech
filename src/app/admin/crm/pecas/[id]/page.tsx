...
import { PartOrderTimeline } from '@/app/admin/crm/components/PartOrderTimeline';
import { PartOrderActions } from './PartOrderActions';
import type { PartOrder, PartOrderStatusValue, ReturnReasonValue } from '@/app/admin/crm/types/database';
...
export default async function PartOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/admin/crm/pecas" className="text-sm text-blue-600 hover:text-blue-700">
          ← Todos os pedidos
        </Link>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4 sm:p-5">
            ...
            {so ? (
              <p className="mt-1 text-slate-900">
                OS{' '}
                <Link href={`/admin/crm/os/${so.id}`} className="font-mono font-semibold text-blue-600 hover:text-blue-700">

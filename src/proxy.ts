 ...
 * CRM novo em /admin/*, que tem seu proprio auth check no
 * src/app/admin/layout.tsx.
 *
 * Este middleware agora so:
 *   1. Redireciona /admin (rota de aviso) e /admin/* para
 *      /admin/login se nao houver cookie de sessao do CRM.
 *   2. Nao tenta autenticar no nivel de middleware (o CRM cuida).
 *
 * Para producao, configure a auth check real no client side
 * via contexto (ja feito em src/app/admin/layout.tsx).
 */
export function proxy(request: NextRequest) {
    ...
    // /admin (sem sub-rota) -> redireciona pro login
